...
?>
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            type: 'pie',
            data: {
                labels: [<?php while($b = mysqli_fetch_array($nama_pekerjaan)) { echo '"' . $b['nama_prod'] . '",'; } ?>], //keterangan nama-nama label],
                datasets: [{
                    label: 'Grafik harga', // judul grafik
                    data: [<?php while($a = mysqli_fetch_array($jumlah)) { echo $a['harga_prod'] . ', '; } ?>], //Data Grafik
                    backgroundColor: [
